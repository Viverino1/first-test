...
const Card = ({data, isPreview}) => {
    ...
    useEffect(() => {
        if(!isPreview){
            ...
            setOwnerPhotoURL("./cosmicLogo.png");
        }
    })
    ...
    return(
        <div className="text-quaternary">
            <div className="relative flex flex-col items-baseline h-96 w-96 rounded-lg bg-primary">
                <div className="flex justify-between items-center w-full h-16 p-2">
                    ...
                    <button className={`${isPreview? "invisible" : "visible"} group flex justify-center items-center w-12 h-12 rounded-3xl transition-all duration-200 border-2 border-secondary hover:bg-secondary`}>
                        <ThreeDotsVertical size={30} onClick={() => setIsCardMenuActive(!isCardMenuActive)}/>
                    </button>
                    <div className={`${isCardMenuActive? "visible" : "hidden"} absolute -right-16 top-0 flex flex-col space-y-2 z-30 w-16 rounded-lg h-fit p-2 bg-primary`}>
...
const MenuElement = ({icon, callback, isSelected}) => {
    return(
        <button className={`${isSelected? "bg-tertiary hover:bg-primary" : "bg-primary hover:bg-secondary"} h-12 w-12 flex justify-center items-center p-2 rounded-lg border-2 border-secondary transition-all duration-300`} onClick={callback}>
            {icon}
        </button>
    )
}
